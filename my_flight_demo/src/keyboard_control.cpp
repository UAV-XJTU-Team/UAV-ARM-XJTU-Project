...
#include "my_flight_demo/flight_control.h"
...
void keyboard_callback(const std_msgs::String::ConstPtr& msg)
{
    ...
    {
        ROS_INFO("please use awsd and jikl");
    }
    control.axes.push_back(V[0]);
    control.axes.push_back(V[1]);
    control.axes.push_back(V[2]);
    control.axes.push_back(V[3]);
    control.axes.push_back(flag);
    ctrlFlyPub.publish(control);
}
...
int main(int argc, char **argv)
{
    ...
    ctrlFlyPub = nh.advertise<sensor_msgs::Joy>("dji_sdk/flight_control_setpoint_generic", 10);
    ...
    // }
    ros::Subscriber keyboardSub = nh.subscribe("keyboard", 10, &keyboard_callback);
    ros::spin();
    return 0;
}
